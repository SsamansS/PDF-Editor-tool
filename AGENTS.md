...
> Клиент НЕ передаёт пути к файлам — только токен `doc`. `doc_id` валидируется regex
> `^[0-9a-f]{32}$`, файлы строго внутри `_uploads/<doc_id>/` (path-traversal невозможен).
> Legacy-эндпоинт `/action` и `_resolve_pdf` удалены (T4) — принимали произвольный путь.
